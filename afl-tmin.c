...
static u8 run_target(char** argv, u8* mem, u32 len, u8 first_run) {
	...
	result = ReadCommandFromPipe(1000);
	if (result == 'K')
	{
...
static void minimize_dir(char** argv, BOOL cmin)
{
	char** nl = 0;
	s32 nl_cnt, good_cnt = 0;
	s32 i,j;
	s32 fd;
	
	ACTF("Scanning '%s'...", in_file);

	/* We use scandir() + alphasort() rather than readdir() because otherwise,
	   the ordering  of test cases would vary somewhat randomly and would be
	   difficult to control. */

	nl_cnt = scandir_sorted((char*)in_file, &nl);
	...
	for (i = 0; i < nl_cnt; i++) {
		...
		u8* fn = alloc_printf("%s\\%s", in_file, nl[i]);
		u8* dfn = alloc_printf("%s\\%s", out_file, nl[i]);
		...
		in_data = ck_alloc_nozero(st_size);
		ck_read(fd, in_data, st_size, fn);
		_close(fd);

		res = run_target(argv, in_data, st_size, 0);
		ck_free(in_data);

		if (cmin)
		{
			if (!_access(dfn, 0)) unlink(dfn);

			if (count_bytes(trace_bits) && has_new_bits(virgin_bits))
			{
				ACTF("'%s' is good, bits count: %d", fn, count_bytes(trace_bits));
				fd = open(dfn, O_WRONLY | O_BINARY | O_CREAT | O_EXCL, 0600);
				if (fd < 0) PFATAL("Unable to create '%s'", dfn);
				ck_write(fd, in_data, st_size, dfn);
				close(fd);
				good_cnt++;
			}
			else
			{
				ACTF("'%s' is useless", fn);
			}
		}
		else
		{
	...
	if (nl) free(nl); /* not tracked */
	ACTF("%d/%d samples are saved", good_cnt, nl_cnt);
}
...
static void write_bitmap(void) {
	...
	fname = alloc_printf("%s\\fuzz_bitmap", out_file);
...
int main(int argc, char** argv) {
	...
	while ((opt = getopt(argc, argv, "+i:o:f:m:t:B:D:cxeQY")) > 0)
		...
		case 'c':
			...
			if (cmin_mode) FATAL("Multiple -c options not supported");
			cmin_mode = 1;
			break;
	...
	if (cmin_mode) {
		fuzz_iterations_max = 10000;
		memset(virgin_bits, 255, MAP_SIZE);
		minimize_dir(use_argv, TRUE);
		destroy_target_process(2000);
		fuzz_iterations_max = 1;
		minimize_dir(use_argv, FALSE);
		write_bitmap();
	}
	else {
		read_initial_file();
